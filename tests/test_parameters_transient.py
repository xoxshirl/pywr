from pywr_adaptive_parameters import TransientDecisionParameter, ScenarioTreeDecisionItem, ScenarioTreeDecisionParameter, TransientScenarioTreeDecisionParameter
from pywr.parameters import ConstantParameter, ConstantScenarioIndexParameter
from pywr.core import ScenarioIndex, Scenario, Model
import pytest
import numpy as np
import pandas
from pywr.recorders import AssertionRecorder, assert_rec
from fixtures import simple_linear_model, simple_storage_model, model
import os
...
class TestTransientDecisionParameter:
    ...
    def test_variable_setup(self, simple_linear_model):
        ...
        np.testing.assert_allclose(p.get_integer_lower_bounds(), [0.0, ])
        np.testing.assert_allclose(p.get_integer_upper_bounds(), [5.0, ])  # 6th index
        ...
        p.set_integer_variables(np.array([1, ], dtype=np.int32))
...
class TestScenarioTreeDecisionParameter:
    ...
    def test_creating_simple_scenario_tree(self, simple_model_with_scenario_tree):
        ...
        # Scenario selector
        indices = np.zeros(scenario.size, dtype=np.int32)
        indices[scenario.size//2:] = 1
        selector = ConstantScenarioIndexParameter(model, scenario, indices)

        # Now create the parameters
        parameter = ScenarioTreeDecisionParameter(model, stage1, selector, factory)

        # There should be one parameter for each of the stages in the tree
        # and one parameter for the scenario selection
        assert len(parameter.children) == 4
    ...
    def test_creating_transient_scenario_tree(self, simple_model_with_scenario_tree):
        ...
        # Now create the parameters
        parameter_all_off = TransientScenarioTreeDecisionParameter(model, stage1, selector, factory, name='all_off')
        parameter_stage1_on = TransientScenarioTreeDecisionParameter(model, stage1, selector, factory, name='stage1_on')
        parameter_stage2_on = TransientScenarioTreeDecisionParameter(model, stage1, selector, factory, name='stage2_on')

        # There should be one parameter for each of the stages in the tree
        # and one parameter for the scenario selection
        assert len(parameter_all_off.children) == 4
        ...
        # Turn on stage 1
        model.parameters['stage1_on.stage 1.binary'].set_integer_variables(np.array([1, ], dtype=np.int32))
        ...
        model.parameters['stage2_on.stage 2a.binary'].set_integer_variables(np.array([1, ], dtype=np.int32))
        model.parameters['stage2_on.stage 2a.transient'].decision_date = '2035-01-01'
        model.parameters['stage2_on.stage 2b.binary'].set_integer_variables(np.array([1, ], dtype=np.int32))
    ...
    @pytest.mark.parametrize('filename', ['simple1_with_tree.json', 'simple1_with_transient_tree.json'])
    def test_from_json(self, filename):

        model = Model.load(os.path.join(TEST_DIR, 'simple1_with_tree.json'))

        parameter = model.parameters['supply1_max_flow']
        # There should be one parameter for each of the stages in the tree
        # and one parameter for the scenario selection
        assert len(parameter.children) == 4
